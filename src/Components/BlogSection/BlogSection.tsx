...
const BlogSection = ({ tagValue }: IBlogSection) => {
  ...
  return (
    <section id={tagValue} className='mt-8 max-w-sm md:max-w-2xl lg:max-w-4xl xl:max-w-5xl '>
      <div className={` flex border bg-zinc-100 border-neutral-300 cursor-pointer md:min-w-[672px] lg:min-w-[896px] xl:min-w-[1024px] bg-gradient-to-r from-cyan-500 to-blue-500 text-gray-200 hover:shadow-xl  shadow-blue-900 ${isHidden ? 'rounded-lg' : 'rounded-t-lg'}`} onClick={handleToggle}>
        <h3 className='ml-2 text-xl p-2'>{tagValue}</h3>
